 ...
 */
public class UiLibRefreshLayout extends ViewGroup {

    private static final String TAG = UiLibRefreshLayout.class.getSimpleName();
    ...
    private UiLibRefreshOnRefreshListener mRefreshListener;

    private UiLibRefreshOnLoadMoreListener mLoadMoreListener;
    ...
    public UiLibRefreshLayout(Context context) {
        this(context, null);
    }

    public UiLibRefreshLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public UiLibRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.UiLibRefreshLayout, defStyleAttr, 0);
        try {
            final int N = a.getIndexCount();
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == R.styleable.UiLibRefreshLayout_refresh_enabled) {
                    setRefreshEnabled(a.getBoolean(attr, true));

                } else if (attr == R.styleable.UiLibRefreshLayout_load_more_enabled) {
                    setLoadMoreEnabled(a.getBoolean(attr, true));

                } else if (attr == R.styleable.UiLibRefreshLayout_swipe_style) {
                    setSwipeStyle(a.getInt(attr, STYLE.CLASSIC));

                } else if (attr == R.styleable.UiLibRefreshLayout_drag_ratio) {
                    setDragRatio(a.getFloat(attr, DEFAULT_DRAG_RATIO));

                } else if (attr == R.styleable.UiLibRefreshLayout_refresh_final_drag_offset) {
                    setRefreshFinalDragOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.UiLibRefreshLayout_load_more_final_drag_offset) {
                    setLoadMoreFinalDragOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.UiLibRefreshLayout_refresh_trigger_offset) {
                    setRefreshTriggerOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.UiLibRefreshLayout_load_more_trigger_offset) {
                    setLoadMoreTriggerOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.UiLibRefreshLayout_swiping_to_refresh_to_default_scrolling_duration) {
                    setSwipingToRefreshToDefaultScrollingDuration(a.getInt(attr, DEFAULT_SWIPING_TO_REFRESH_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.UiLibRefreshLayout_release_to_refreshing_scrolling_duration) {
                    setReleaseToRefreshingScrollingDuration(a.getInt(attr, DEFAULT_RELEASE_TO_REFRESHING_SCROLLING_DURATION));

                } else if (attr == R.styleable.UiLibRefreshLayout_refresh_complete_delay_duration) {
                    setRefreshCompleteDelayDuration(a.getInt(attr, DEFAULT_REFRESH_COMPLETE_DELAY_DURATION));

                } else if (attr == R.styleable.UiLibRefreshLayout_refresh_complete_to_default_scrolling_duration) {
                    setRefreshCompleteToDefaultScrollingDuration(a.getInt(attr, DEFAULT_REFRESH_COMPLETE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.UiLibRefreshLayout_default_to_refreshing_scrolling_duration) {
                    setDefaultToRefreshingScrollingDuration(a.getInt(attr, DEFAULT_DEFAULT_TO_REFRESHING_SCROLLING_DURATION));

                } else if (attr == R.styleable.UiLibRefreshLayout_swiping_to_load_more_to_default_scrolling_duration) {
                    setSwipingToLoadMoreToDefaultScrollingDuration(a.getInt(attr, DEFAULT_SWIPING_TO_LOAD_MORE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.UiLibRefreshLayout_release_to_loading_more_scrolling_duration) {
                    setReleaseToLoadingMoreScrollingDuration(a.getInt(attr, DEFAULT_RELEASE_TO_LOADING_MORE_SCROLLING_DURATION));

                } else if (attr == R.styleable.UiLibRefreshLayout_load_more_complete_delay_duration) {
                    setLoadMoreCompleteDelayDuration(a.getInt(attr, DEFAULT_LOAD_MORE_COMPLETE_DELAY_DURATION));

                } else if (attr == R.styleable.UiLibRefreshLayout_load_more_complete_to_default_scrolling_duration) {
                    setLoadMoreCompleteToDefaultScrollingDuration(a.getInt(attr, DEFAULT_LOAD_MORE_COMPLETE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.UiLibRefreshLayout_default_to_loading_more_scrolling_duration) {
                    setDefaultToLoadingMoreScrollingDuration(a.getInt(attr, DEFAULT_DEFAULT_TO_LOADING_MORE_SCROLLING_DURATION));

                } else if (attr == R.styleable.UiLibRefreshLayout_template) {
                    setTemplate(a.getInt(attr, DEFAULT_TEMPLATE));
                }
            }
        } finally {
            a.recycle();
    ...
    protected void onFinishInflate() {
        ...
        if (childNum == 0) {
            ...
            for (int i = 0; i < childNum; i++) {
                ...
                if (view instanceof UiLibRefreshHeaderLayout) {
                    mHeaderView = view;
                } else if (view instanceof UiLibRefreshFooterLayout) {
                    mFooterView = view;
                } else {
                    mTargetView = view;
                }
...
//            mFooterView = findViewById(R.id.swipe_load_more_footer);
        ...
        if (mHeaderView != null && mHeaderView instanceof UiLibRefreshTrigger) {
            mHeaderView.setVisibility(GONE);
        }
        if (mFooterView != null && mFooterView instanceof UiLibRefreshTrigger) {
    ...
    /**
     * set refresh header view, the view must at lease be an implement of {@code UiLibRefreshRefreshInterface}.
     * the view can also implement {@code UiLibRefreshTrigger} for more extension functions
     *
     * @param view
     */
    public void setRefreshHeaderView(View view) {
        if (view instanceof UiLibRefreshRefreshInterface) {
            ...
            Log.e(TAG, "Refresh header view must be an implement of UiLibRefreshRefreshInterface");
        }
    }
    ...
    public void setLoadMoreFooterView(View view) {
        if (view instanceof UiLibRefreshLoadMoreInterface) {
    ...
    public void setTemplate(int template) {
        ...
        switch (template) {
            case TEMPLATE.NORMAL:
                headerResId = R.layout.uilib_layout_refresh_header;
                footerResId = R.layout.uilib_layout_refresh_footer;
                break;
        }
//        UiLibRefreshHeaderLayout head = new UiLibRefreshHeaderLayout(getContext());
//        FrameLayout.LayoutParams p1 = new FrameLayout.LayoutParams(LayoutParams.MATCH_PARENT, 60);
//        head.setLayoutParams(p1);
//        head.setBackgroundColor(0xffff0000);
//        UiLibRefreshFooterLayout footer = new UiLibRefreshFooterLayout(getContext());
//        FrameLayout.LayoutParams p2 = new FrameLayout.LayoutParams(LayoutParams.MATCH_PARENT, 60);
//        footer.setLayoutParams(p2);
//        footer.setBackgroundColor(0xffffff00);
//
//        setRefreshHeaderView(head);
//        setLoadMoreFooterView(footer);

        View head = LayoutInflater.from(getContext()).inflate(headerResId, null);
        View footer = LayoutInflater.from(getContext()).inflate(footerResId, null);
        FrameLayout.LayoutParams p1 = new FrameLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
        FrameLayout.LayoutParams p2 = new FrameLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
...
//        setRefreshHeaderView(LayoutInflater.from(getContext()).inflate(headerResId, null));
//        setLoadMoreFooterView(LayoutInflater.from(getContext()).inflate(footerResId, null));
    }

    /**
     * set an {@link UiLibRefreshOnRefreshListener} to listening refresh event
     *
     * @param listener
     */
    public void setOnRefreshListener(UiLibRefreshOnRefreshListener listener) {
        this.mRefreshListener = listener;
    }

    /**
     * set an {@link UiLibRefreshOnLoadMoreListener} to listening load more event
     *
     * @param listener
     */
    public void setOnLoadMoreListener(UiLibRefreshOnLoadMoreListener listener) {
    ...
    RefreshTriggerCallback mRefreshCallback = new RefreshTriggerCallback() {
        @SuppressLint("WrongConstant")
        @Override
        public void onPrepare() {
            if (mHeaderView != null && mHeaderView instanceof UiLibRefreshTrigger && STATUS.isStatusDefault(mStatus)) {
                mHeaderView.setVisibility(VISIBLE);
                ((UiLibRefreshTrigger) mHeaderView).onPrepare();
            }
        }
        ...
        public void onMove(int y, boolean isComplete, boolean automatic) {
            if (mHeaderView != null && mHeaderView instanceof UiLibRefreshTrigger && STATUS.isRefreshStatus(mStatus)) {
                if (mHeaderView.getVisibility() != VISIBLE) {
                    mHeaderView.setVisibility(VISIBLE);
                }
                ((UiLibRefreshTrigger) mHeaderView).onMove(y, isComplete, automatic);
            }
        }

        @Override
        public void onRelease() {
            if (mHeaderView != null && mHeaderView instanceof UiLibRefreshTrigger && STATUS.isReleaseToRefresh(mStatus)) {
                ((UiLibRefreshTrigger) mHeaderView).onRelease();
            }
        }
        ...
        public void onRefresh() {
            if (mHeaderView != null && STATUS.isRefreshing(mStatus)) {
                if (mHeaderView instanceof UiLibRefreshRefreshInterface) {
                    ((UiLibRefreshRefreshInterface) mHeaderView).onRefresh();
                }
        ...
        public void onComplete() {
            if (mHeaderView != null && mHeaderView instanceof UiLibRefreshTrigger) {
                ((UiLibRefreshTrigger) mHeaderView).onComplete();
            }
        }
        ...
        public void onReset() {
            if (mHeaderView != null && mHeaderView instanceof UiLibRefreshTrigger && STATUS.isStatusDefault(mStatus)) {
                ((UiLibRefreshTrigger) mHeaderView).onReset();
                mHeaderView.setVisibility(GONE);
            }
        }
    };
    ...
    LoadMoreCallback mLoadMoreCallback = new LoadMoreCallback() {
        ...
        @Override
        public void onPrepare() {
            if (mFooterView != null && mFooterView instanceof UiLibRefreshTrigger && STATUS.isStatusDefault(mStatus)) {
                mFooterView.setVisibility(VISIBLE);
                ((UiLibRefreshTrigger) mFooterView).onPrepare();
            }
        }
        ...
        @Override
        public void onMove(int y, boolean isComplete, boolean automatic) {
            if (mFooterView != null && mFooterView instanceof UiLibRefreshTrigger && STATUS.isLoadMoreStatus(mStatus)) {
                if (mFooterView.getVisibility() != VISIBLE) {
                    mFooterView.setVisibility(VISIBLE);
                }
                ((UiLibRefreshTrigger) mFooterView).onMove(y, isComplete, automatic);
            }
        }

        @Override
        public void onRelease() {
            if (mFooterView != null && mFooterView instanceof UiLibRefreshTrigger && STATUS.isReleaseToLoadMore(mStatus)) {
                ((UiLibRefreshTrigger) mFooterView).onRelease();
            }
        }
        ...
        public void onLoadMore() {
            if (mFooterView != null && STATUS.isLoadingMore(mStatus)) {
                if (mFooterView instanceof UiLibRefreshLoadMoreInterface) {
                    ((UiLibRefreshLoadMoreInterface) mFooterView).onLoadMore();
                }
        ...
        public void onComplete() {
            if (mFooterView != null && mFooterView instanceof UiLibRefreshTrigger) {
                ((UiLibRefreshTrigger) mFooterView).onComplete();
            }
        }
        ...
        public void onReset() {
            if (mFooterView != null && mFooterView instanceof UiLibRefreshTrigger && STATUS.isStatusDefault(mStatus)) {
                ((UiLibRefreshTrigger) mFooterView).onReset();
                mFooterView.setVisibility(GONE);
            }
        }
    };
     ...
     */
    abstract class RefreshTriggerCallback implements UiLibRefreshTrigger, UiLibRefreshRefreshInterface {
    }

    /**
     * load more event callback
     */
    abstract class LoadMoreCallback implements UiLibRefreshTrigger, UiLibRefreshLoadMoreInterface {
    }
    ...
    private class AutoScroller implements Runnable {
        ...
        public void run() {
            ...
            if (finish) {
                ...
                mmLastY = currY;
                UiLibRefreshLayout.this.autoScroll(yDiff);
                post(this);
            }
        }
